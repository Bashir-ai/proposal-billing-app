...
export function BenefitsSection({ userId, startDate, endDate, isAdmin }: BenefitsSectionProps) {
  ...
  const fetchOccurrences = async () => {
    try {
      ...
      const response = await fetch(
        `/api/users/${userId}/fringe-benefit-occurrences?${params.toString()}`
      )
      const data = await response.json()
      const list = data.occurrences || []

      // Keep the same category filter UX as the definitions list.
      const filtered =
        selectedCategory && selectedCategory.length > 0
          ? list.filter((o: any) => o.fringeBenefit?.category === selectedCategory)
          : list

      setOccurrences(filtered)
    } catch (error) {
      console.error("Error fetching benefit occurrences:", error)
    } finally {
  ...
  const handlePayOccurrence = async (occurrence: any) => {
    if (!occurrence?.id) return
    if (!occurrence?.remainingAmount || occurrence.remainingAmount <= 0) return

    const amount = parseFloat(occurrence.remainingAmount)
    if (isNaN(amount) || amount <= 0) return

    if (
      !confirm(
        `Pay benefit occurrence of "${occurrence.fringeBenefit?.description || occurrence.id}" for ${formatCurrency(
          amount
        )}?`
      )
    ) {
    ...
    try {
      setPayingOccurrenceId(occurrence.id)

      const res = await fetch(`/api/users/${userId}/payouts/record`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          kind: "FRINGE_BENEFIT_OCCURRENCE",
          fringeBenefitOccurrenceId: occurrence.id,
          amount,
      ...
      if (!res.ok) {
        throw new Error(data?.error || "Failed to record benefit payout")
      }

      await fetchOccurrences()
    } catch (error: any) {
      alert(error?.message || "Failed to record benefit payout")
    } finally {
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="mt-6">
        <CardHeader>
          <CardTitle>Benefit Occurrences</CardTitle>
          <CardDescription>Recurring benefits materialized into due occurrences</CardDescription>
        </CardHeader>
        <CardContent>
          {occurrencesLoading ? (
            <div>Loading benefit occurrences...</div>
          ) : occurrences.length === 0 ? (
            <p className="text-gray-500">No benefit occurrences found for the selected period.</p>
          ) : (
                ...
                >
                  <div>
                    <p className="font-semibold">
                      {formatDate(occ.occurrenceDate)} - {occ.fringeBenefit?.description || occ.id}
                    </p>
                    <p className="text-sm text-gray-600">
                      Amount: {formatCurrency(occ.amount)} ({occ.fringeBenefit?.category || "OTHER"})
                    </p>
                    <p className="text-sm text-gray-600">
                      Remaining: {formatCurrency(occ.remainingAmount)} - Status: {occ.status}
                    </p>
                  </div>
                  <div className="flex flex-col items-end gap-2">
                    {isAdmin && occ.remainingAmount > 0 && (
                      <Button
                        size="sm"
                        variant="outline"
                        disabled={payingOccurrenceId === occ.id}
                        onClick={() => handlePayOccurrence(occ)}
                      >
                        {payingOccurrenceId === occ.id ? "Paying..." : "Pay"}
                      </Button>
                    )}
                  </div>
                </div>
