...
export function OutstandingInvoiceAlert({ invoice }: OutstandingInvoiceAlertProps) {
  ...
  const now = new Date()
  const isEligibleStatus = invoice.status !== "PAID" && invoice.status !== "CANCELLED" && invoice.status !== "WRITTEN_OFF"
  const isOutstanding = isEligibleStatus && dueDate < now && invoice.becameOutstandingAt
